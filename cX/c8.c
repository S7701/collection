...
int run(int argc, char **argv) {
  ...
  *--sp = (int)pp;
  ...
  while (1) {
    ...
    if (d >= data + DataSz) { printf("FATAL: data buffer overflow (pc %p, d %p, data %p, DataSz 0x%08x)\n", pc, d, data, DataSz); exit(-1); }
    if (sp < stack || sp >= stack + StackSz) { printf("FATAL: stack overflow (pc %p, sp %p, stack %p, StackSz 0x%08x)\n", pc, sp, stack, StackSz); exit(-1); }
...
int main(int argc, char **argv) {
  ...
  if (!(sym = malloc(SymSz))) { printf("FATAL: could not malloc(%d) symbol table\n", SymSz); return -1; }
  memset(sym, 0, SymSz);
  if (!(le = e = code = malloc(CodeSz))) { printf("FATAL: could not malloc(%d) code buffer\n", CodeSz); return -1; }
  memset(e, 0, CodeSz);
  if (!(d = data = malloc(DataSz))) { printf("FATAL: could not malloc(%d) data buffer\n", DataSz); return -1; }
  memset(d, 0, DataSz);
  if (!(stack = malloc(StackSz))) { printf("FATAL: could not malloc(%d) stack buffer\n", StackSz); return -1; }
