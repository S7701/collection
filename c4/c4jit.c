...
enum Token { Num = 128, Fun, Glo, Loc, Id,
...
void expr(int lev) {
  ...
  if (!tk) { printf("%d: unexpected eof in expression\n", line); exit(-1); }
    ...
    *++e = 0xb8; *(int *)e = val; e = e + 4; if (src) { printf("  movl $%d, \%eax\n", val); }
    next(); ty = INT;
  }
    ...
    *++e = 0xb8; *(int *)e = (ty == CHAR) ? sizeof(char) : sizeof(int); e = e + 4; if (src) { printf("  movl $%d, \%eax\n", val); }
    ty = INT;
  }
...
int main(int argc, char **argv) {
  ...
  i = OPEN; while (i <= EXIT) { next(); id[Class] = Fun; id[Type] = INT; id[Val] = i++; } // add library to symbol table
